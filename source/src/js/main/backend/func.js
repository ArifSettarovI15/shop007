...
function SearchModel(item) {
  var val=item.val();

  var data_array={};
  data_array['action']='search';
  data_array['title']=val;

  var options={};
  options['AfterDone']=SearchModelDone;
  SendAjaxRequest(
    {
      'url':'/manager/shop/models/',
      'data':data_array,
      'options':options,
      'onlyOne':true
    }
...
function SearchModelDone(response,ajax_config,textStatus,jqXHR) {
  $('[data-block="models"].search-block-items .item-search__result').html(response.html);
}
...
function FilterTableDataDone(response,ajax_config,textStatus,jqXHR) {
  ...
  if (ajax_config['options']['obj'].attr('data-final')) {
    var ff=window[ajax_config['options']['obj'].attr('data-final')];
    ff();
  }
}
...
function InsertRouteAction(data) {
    var html='<option value="">Без action</option>';
    $(data.list).each(function(index,value) {
        html=html+'<option value="'+value+'">'+value+'</option>';
    });
...
function confirmRecover() {
    if (confirm("Вы подтверждаете восстановление?")) {
        return true;
    } else {
        return false;
    }
