...
class UserClass
{
    ...
    function CheckProfileFields ($data) {
        ...
        else if ($data['profile_lastname']=='') {
            $error=$this->lang['enter_lastname'];
            $field='lastname';
        }
        return array($error,$field);
    }
	...
	function UpdateUserManager ($user_id, $manager_id=0){
		return $this->db->query_write("UPDATE `users` SET
        `user_manager_id`=".$this->db->sql_prepare('1011')."
        WHERE `user_id`=".$this->db->sql_prepare($user_id));
	}
	/**
     * @param $type string
    ...
    function PrepareUsersWhere ($filter_options){
        ...
        if (intval($filter_options['user_id'])>0){
	     ...
	     if (isset($filter_options['user_manager_id']) and $filter_options['user_manager_id']!=-1){
		     if ($sql != '') {
			     $sql .= ' AND ';
		     } else {
			     $sql .= ' WHERE ';
		     }
		     $sql.="`users`.`user_manager_id`=".$this->db->sql_prepare(1011);
	     }
        ...
        if ($filter_options['name']!=''){
            ...
            $sql.=' `users_profile`.profile_company  LIKE "%'.$this->db->escape_string_like($filter_options['name']).'%"';
        }
        ...
        elseif ($filter_options['order']=='name'){
	        $filter_options['order_sql']=' `users_profile`.profile_lastname, `users_profile`.profile_name';
        }
        else {
            $filter_options['order_sql']=' `users`.`user_id`';
        }
    ...
    function GetUsersFromDb ($filter_options=array(), $count, $start_page){
        ...
        return $this->db->query_read("SELECT `users`.*, `users_profile`.*, manager_data.profile_lastname as manager_lastname, 
    ...
    function GetProfilePhotoUrl ($photo_path){
        if ($photo_path=='') {
            return BASE_URL.'/assets/images/core/no-photo.png';
        }
        else {
    ...
    function GetUserProfileFromDb ($user_id){
        return $this->db->query_first("SELECT `users_profile`.*
 		FROM `users_profile`
	...
	function getUserBrandSpecialOffer($brand_id) {
    	$group_id=$this->registry->user_profile['profile_discounts'][$brand_id]['group_id'];
    	return $this->registry->shop->getOfferPriceById($group_id);
	}
	function getUserBrandSpecialItem($brand_id) {
		$group_id=$this->registry->user_profile['profile_discounts'][$brand_id]['group_id'];
		return $this->registry->shop->getItemPriceById($group_id);
	}

	function checkUserBrandSpecialPrice($brand_id) {
	...
	function getDiscount($price,$brand_id=0) {
    	/*if (isset($this->registry->user_profile['profile_discounts'][$brand_id])) {
	    	...
	    	if ($this->registry->user_profile['profile_discount']) {
