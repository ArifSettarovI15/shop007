...
class SpravClass
{
	...
	public function GetSpravValues ($filter_options=array(), $count='all', $start_page=0) {
		...
		while ($result_item = $this->db->fetch_array($result))
		{
			...
			if ($result_item['sprav_id']==3 or $result_item['sprav_id']==2) {
				$result_item['svid_title']=str_replace(',','.',$result_item['svid_title']);
			}
		...
		if ($filter_options['group_array']=='sprav_id') {
			foreach ($array as $key=>$v) {
				usort( $array[ $key ], function ( $a, $b ) {
					return strnatcasecmp( $a['svid_title'], $b['svid_title'] );
				} );

				if ($key==2) {
					$new=array();
					foreach ($array[ $key ] as $a1=>$a2) {
						if ($a2['svid_title']>=130) {
							$new[$a1]=$a2;
						}
					}
					foreach ($array[ $key ] as $a1=>$a2) {
	...
	private function PrepareSpravValuesData ($result_item,$full=0) {
		...
		$result_item['sprav_icon_url'] = $this->registry->files->GetImageUrl($result_item,'medium');
	...
	private function PrepareSpravValuesWhere ($filter_options=array()){
        ...
        if ($filter_options['svids_in']!='' and $filter_options['svids_in'] ) {
            if ($sql != '') {
                $sql .= ' AND ';
            } else {
                $sql .= ' WHERE ';
            ...
            $string = '(';
            $index=1;
            $sql.="`core_sprav_values`.`svid` IN (".implode(',',$filter_options['svids_in']).')';
        }
    ...
    public function getSpravIdValues($sprav_id, $filters='') {
        $filter_options=array();
        $filter_options['sprav_id']=$sprav_id;
        $filter_options['svid_status']=true;
        $filter_options['svids_in']=$filters['svids'];
