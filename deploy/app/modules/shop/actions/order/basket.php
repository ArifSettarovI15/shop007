...
if ($Main->GPC['action'] == 'process_order') {
    ...
    $kontragent  = [
            ...
            "Name"=> $Main->GPC['name'],
            "ShortName"=> $Main->GPC['name'],
            "Type"=> 1
    ];
    $kontragent = json_encode($kontragent);

    $curl = curl_init();
    curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://restapi.moedelo.org/kontragents/api/v1/kontragent/',
    ...
    $kontragent = json_decode($response);
    ...
    $array = [
        "DeliveryInfo" => [
            ...
            "DeliveryDate"=> date('Y-m-d\TH:i:s', strtotime(date('Y-m-d\TH:i:s',time().' + 3 days'))),
            "RecipientInfo" => $Main->GPC['name'],
        ],
        "Kontragent" => [
            "KontragentId"=> $kontragent->Id,
            "KontragentName"=> $Main->GPC['name'],
            "KontragentPhone"=> $Main->GPC['phone']
        ],
        "Items"=>$items_data,
        "IsReserved" => false,
        "NdsPositionType" => 1,
    ...
    $curl = curl_init();
    curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://restapi.moedelo.org/stockorders/api/v1/CustomerOrders/',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
    ...
    if (!$response->data){
        $array['status'] = false;
        $array['text'] = "Не удалось сознать заказ, обратитесь в поддержку.";
    } else {
        $array['status'] = true;
        $array['text'] = "Ваш заказ успешно создан.";
        $array['redirect'] = "/";
