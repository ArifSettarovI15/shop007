...
elseif ($Main->user_info['user_role_id']==3) {
	if ($user['user_manager_id']==0 or $user['user_manager_id']==$Main->user_info['user_id']) {
...
if ($Main->user_info['user_role_id']==2) {
	...
	if ( $Main->GPC['action'] == 'save_user_vendor_value' ) {
		...
		$profile['profile_discounts'][ $Main->GPC['value'] ]['group_id'] = $Main->GPC['group_id'];
