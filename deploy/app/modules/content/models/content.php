...
class ContentClass
{
    ...
    function GetContentList ($filter_options, $count=10, $start_page=0) {
        $array=array();
        $result=$this->GetContentListFromDb($filter_options, $count, $start_page);
        while ($result_item = $this->db->fetch_array($result))
        {
            $result_item = $this->PrepareContentData($result_item);
            $array[]=$result_item;
        }

        return $array;
    }
    function GetContentListFromDb ($filter_options, $count, $start_page) {
        $sql=$this->PrepareContentListWhere($filter_options);
        if ($count=='all') {
            $sql_limit="";
        }
        else {
...
thumb3.file_sizes as thumb3_file_sizes, thumb3.file_folder as thumb3_file_folder, thumb3.file_name as thumb3_file_name
    ...
    function PrepareContentListWhere ($filter_options=array()) {
        $sql='';
        ...
        if ($filter_options['notId']!='' ){
            ...
            $sql.="`core_content`.`content_id`!=".$this->db->sql_prepare($filter_options['notId']);
        }
        ...
        if ($filter_options['not_type']!='') {
            if ($sql != '') {
                $sql .= ' AND ';
            } else {
                $sql .= ' WHERE ';
            }
            $sql .= "`core_content`.`content_type`!=" . $this->db->sql_prepare($filter_options['not_type']);
        }
        $gr='';
...
thumb3.file_sizes as thumb3_file_sizes, thumb3.file_folder as thumb3_file_folder, thumb3.file_name as thumb3_file_name
    ...
    function AddContent ($content_status,$content_title,$content_url,$content_type,$content_short,
        $meta_title,$meta_keywords,$meta_desc,$text_id,$photo_id,$photo_id3,$time,$date,$content_cat,$template_name,$vendor_id,$share_pos,$dop,$time_end){
        $this->db->query_write("INSERT INTO `core_content`
                ...
                `content_thumb_id3`,`head_title`,`head_desc`,`head_keywords`,`content_date`,`content_cat`,`content_template`,`content_vendor_id`,`share_position`,`content_dop`,`content_time_end`)
                VALUES (
    ...
    function UpdateContent ($content_id,$content_status,$content_title,$content_url,$content_type,$content_short,$meta_title,$meta_keywords,$meta_desc,$text_id,$photo_id,$photo_id3,$content_cat,$template_name,$time,$vendor_id,$share_pos,$dop,$time_end){
        ...
        `content_thumb_id3`=".$this->db->sql_prepare($photo_id3).",
        ...
        `share_position`=".$this->db->sql_prepare($share_pos).",
