...
class DbColumn {
	...
	public function notInValues( $value ) {
		if (count($value)==1) {
			$this->setValue(array_values($value)[0]);
			$this->dop = 'not';
		}
...
class DbDataModel {
	 ...
	 * @param string $having
	 */
	public function setHaving( string $having ): void {
		$this->having = $having;
	}
...
class DbData
{
	...
	private function SqlQuery ($type=false,$custom_query='') {
		...
		elseif ($type==QUERY_TYPE_SELECT OR $type==QUERY_TYPE_FIRST) {
			if (count($select_array)>0) {
				$kk='  ';
				if ($this->model->getStr()) {
					$kk=' STRAIGHT_JOIN';
				}
				$select='SELECT'.$kk.' '.implode(',',$select_array).' FROM ';
			}
			else {
				$select='SELECT * FROM ';
			}
	 ...
	 */
	private function GetSqlResultRaw ($type=QUERY_TYPE_FIRST,$custom_query='') {

		return $this->SqlQuery($type,$custom_query);
	...
	private function GetSqlResult ($type=QUERY_TYPE_FIRST,$custom_query='') {
		...
		if ($_SERVER['REMOTE_ADDR']=='89.107.138.42') {
			//echo $sql;
		}
		if ($sql) {
	...
	public function GetMinMaxSql ($elem) {
		$elem=$this->model->GetTableItemName($elem);
		return $this->GetSqlResultRaw(QUERY_TYPE_MINMAX,$elem);
	}
