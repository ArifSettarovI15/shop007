...
class ShopProductsModel extends DbDataModel {
	...
	public function SetJoinBasket() {
		...
		$this->setSelectField(' `shop_item_basket`.*, `shop_offers`.*');
		$this->setJoin(' 
		LEFT JOIN `shop_item_basket` ON shop_item_basket.`basket_item_id`
        LEFT JOIN `shop_offers` ON `shop_offers`.offer_id AND shop_offers.offer_id = shop_item_basket.`basket_item_id`
		');
		$this->SetJoinCats();
		$this->SetJoinVendors();
...
//			$result_item['item_amount_zak']=intval($result_item['item_amount'])-intval($result_item['item_amount_nal']);
...
//		}
		...
		if (isset($result_item['offer_price'])) {
			...
			$result_item['offer_amount_zak']=intval($result_item['offer_amount'])-intval($result_item['offer_amount_nal']);
	...
	//	$result_item['offer_price_before']=0;
		...
		if ($result_item['offer_icon']) {
			...
			$result_item=$this->registry->files->FilePrepare($result_item,'icon_offer_');
			$result_item['item_original_url'] = $this->registry->files->GetImageUrl($result_item,'original',0,'icon_offer_');
		}
		else {
			...
			$result_item=$this->registry->files->FilePrepare($result_item,'icon_');
			$result_item['item_original_url'] = $this->registry->files->GetImageUrl($result_item,'original',0,'icon_');
		}
		...
		if ($result_item['item_id']) {
			...
			$result_item['item_full_url'] = BASE_URL.'/'.$result_item['cat_url'].'/'.$result_item['item_url'].'/';
	...
	function GetBasket () {
		...
		foreach ($items as $k=>$item) {
		    if ($item['offer_item_id']){
                $item_res = $this->GetItemById($item['offer_item_id']);
                $item['item_icon_url'] = $item_res['item_icon_url'];
            }

			$total=$total+$item['basket_count'];
			$item['basket_sub_total']=$item['basket_count']*$item['basket_price'];
			$sum=$sum+$item['basket_sub_total'];
			$new_items[$item['offer_id']]=$item;
		}
//        print_r($items[1]);exit;
		$items=$new_items;
